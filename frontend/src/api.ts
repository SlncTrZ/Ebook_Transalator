...
export const createGlossary = (bookId: number, source: string, target: string, notes = '') =>
  request<{ id: number }>('/glossary', {
    method: 'POST',
    body: JSON.stringify({ book_id: bookId, source_term: source, target_term: target, notes }),
  });
export const deleteGlossary = (id: number) =>
  request<{ ok: boolean }>(`/glossary/${id}`, { method: 'DELETE' });

// Translation
export const startTranslate = (filePath: string, apiKey: string, model: string, category: string) =>
  request<{ book_id: number; status: string }>('/translate/start', {
    method: 'POST',
    body: JSON.stringify({ file_path: filePath, api_key: apiKey, model, category }),
  });

export const cancelTranslate = () =>
  request<{ status: string }>('/translate/cancel', { method: 'POST' });

export const translateProgress = (bookId: number, onProgress: (data: ProgressData) => void, onComplete: () => void, onError: (err: string) => void) => {
