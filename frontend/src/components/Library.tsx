    ...
    <div className="library">
      <h2>📚 Book Library</h2>
    			...
    			<div className="import-bar">
    				...
    				/>
    				<button onClick={() => doImport(importPath)} disabled={!importPath.trim() || importing}>
    					{importing ? '⏳...' : '+ Import'}
    				</button>
    				<button
      ...
      <div className="import-presets" style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
        {TEST_FILES.map((f) => {
          ...
          <tbody>
            {books.map((book) => (
              <tr key={book.id} className={selectedBook?.id === book.id ? 'selected' : ''}>
                ...
                <td>{book.total_chunks > 0 ? `${book.done_chunks}/${book.total_chunks}` : '—'}</td>
                <td><button onClick={() => onSelectBook(book)}>Translate</button></td>
              </tr>
            ))}
          </tbody>
        </table>
