...
import { startTranslate, cancelTranslate, translateProgress, exportBook } from '../api';
...
export function TranslateView({ book, apiKey, model }: TranslateViewProps) {
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      const result = await startTranslate(book.file_path, apiKey, model, category);
  ...
  if (!book) {
    return (
      <div className="translate-view">
        <h2>🌐 Translate</h2>
        <p className="muted">Select a book from the Library tab to start translating.</p>
      </div>
    );
  }
